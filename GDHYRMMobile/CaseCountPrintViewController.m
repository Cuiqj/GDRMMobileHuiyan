...
- (id)dataForPDFTemplate {
    ...
    NSString *description = [CaseProveInfo eventDescForTypePeiWithCase2:caseInfo];
    NSString *caseNo = [NSString stringWithFormat:@"%@-%@",caseInfo.case_mark2,[NSString stringWithFormat:@"%02d", caseInfo.case_mark3.intValue]];
    id citizenData = @{};
    Citizen  *citizen = [Citizen citizenForCitizenName:nil nexus:@"当事人" caseId:self.caseID];
    if (citizen) {
        citizenData = @{
    ...
    /* 若不足10个，用空数据补足 */
    for (int i = 10-emptyItemCnt; i < 10; i++) {
        [itemsData addObject:@{}];
    }
