...
+ (NSString *)eventDescForTypePeiWithCase2:(CaseInfo *)caseInfo{
    NSString *roadName=[RoadSegment roadNameFromSegment:caseInfo.roadsegment_id];
    NSString *caseDescString=@"";
    NSManagedObjectContext *context=[[AppDelegate App] managedObjectContext];
    ...
    if (citizenArray.count>0) {
        if (citizenArray.count==1) {
            Citizen *citizen=[citizenArray objectAtIndex:0];
            caseDescString = [NSString stringWithFormat:@"%@，驾驶员：%@；驾驶%@，车型：%@，经过%@%@%@，因%@发生事故，造成公路路产损失，详细如下：",happenDate,citizen.party,citizen.automobile_number,citizen.automobile_pattern,roadName,caseInfo.side,stationString,caseInfo.case_reason];
